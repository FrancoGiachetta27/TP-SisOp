...
int assign_block_size(int file_size)
{
    ...
    log_info(utils->logger, "Acceso FAT - Entrada: %d - Valor: %d", current_block, UINT32_MAX);
...
void *read_file(char *file_name, int seek)
{
    ...
    else
    {
        for (int i = 0; i < block_to_read; i++)
        {
            ...
            if (fat_block == NULL)
            {
                log_debug(utils->logger, "[READ] - BLOQUE %d NULO", index_fat);
            }
...
void write_file(char *file_name, int seek, void *data)
{
    ...
    else
    {
        for (int i = 0; i <= block_to_write; i++)
        {
            ...
            if (fat_block == NULL)
            {
                log_debug(utils->logger, "[WRITE] - BLOQUE %d NULO", index_fat);
            }
