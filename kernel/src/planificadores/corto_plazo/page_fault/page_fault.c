...
void* treat_page_fault(t_pf* pf_info) {
    ...
    if (op_code != PAGE_FAULT_COMMAND) {
        log_warning(pf_info->memory_socket, "Invalid op_code %d", op_code);
    }
    int buffer = receive_buffer(pf_info->memory_socket, pf_info->logger);
    if (buffer != PAGE_FAULT_COMMAND) {
        log_error(pf_info->memory_socket, "Fail in response of page fault %d", buffer);
    }
    free(buffer);
    pthread_mutex_lock(&cola_interrupt);
